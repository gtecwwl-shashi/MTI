...
st.markdown("### 🏛️ MTI Pathway Master Flow – Infection Sciences Model")
st.markdown("---")

def render_pillar(code, height=750):
    html = f"""
    <div style="background: white; border: 1px solid #1a3a5f; padding: 15px; border-radius: 10px; height: {height}px; overflow-y: auto; box-shadow: 2px 2px 5px rgba(0,0,0,0.05);">
        <div class="mermaid">
            {code}
        </div>
    </div>
    <script type="module">
        ...
        mermaid.initialize({{ 
            ...
            themeVariables: {{ 'fontSize': '14px', 'fontFamily': 'Arial', 'primaryColor': '#ffffff' }}
        }});
    </script>
    """
